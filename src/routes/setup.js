...
const { normalizeRole, normalizeRoomName, normalizeUsername } = require('../lib/normalize');
...
const APP_NAME = process.env.APP_NAME || 'raspi-chat';
...
function renderEnvFile(values) {
  return [
    '# Generated by the raspi-chat setup wizard',
    '',
    '# Core app',
...
function setupDefaults(request) {
  ...
  return {
    ...
    adminUsername: normalizeUsername(envValues.DEFAULT_ADMIN_USERNAME || adminUser?.username || 'admin') || 'admin',
...
function renderFinishScript({ serviceFile, nginxFile, networkMode }) {
  const lines = [
    ...
    '',
    'if [ "$(id -u)" -ne 0 ]; then',
    '  echo "Run this script with sudo: sudo bash finish-setup.sh" >&2',
    '  exit 1',
    'fi',
    '',
  ...
  if (networkMode === 'nginx') {
    lines.push(
      '',
      `cp "${nginxFile}" /etc/nginx/sites-available/raspi-chat.conf`,
      'ln -sf /etc/nginx/sites-available/raspi-chat.conf /etc/nginx/sites-enabled/raspi-chat.conf',
      'nginx -t && systemctl reload nginx'
    );
  }
  lines.push('', 'echo "Setup complete: the raspi-chat service is running."', '');
  return lines.join('\n');
}
